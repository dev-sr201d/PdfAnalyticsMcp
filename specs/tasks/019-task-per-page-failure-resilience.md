...
2. The catch must not catch `ArgumentException` itself (to avoid masking validation errors from `ValidatePageNumber`) or `OperationCanceledException` (to avoid masking MCP client cancellation — per FRD-007 FR #10).
...
4. The catch must not catch `ArgumentException` or `OperationCanceledException` itself.
...
6. The catch must not catch `ArgumentException` or `OperationCanceledException` itself.
...
- [ ] `OperationCanceledException` from MCP client cancellation is NOT caught by the per-page error handlers — cancellation propagates correctly (per FRD-007 FR #10).
...
- Add unit tests for each service that verify the per-page error handling:
  ...
  - Verify that `OperationCanceledException` is not caught by the per-page handlers.

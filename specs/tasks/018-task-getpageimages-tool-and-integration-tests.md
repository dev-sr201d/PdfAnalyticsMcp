# Task 018: GetPageImages Tool and Integration Tests
...
> **Note:** The tool class (`GetPageImagesTool`) was created alongside the service in Task 017. This task focuses on verifying it via integration tests and confirming tool discovery, parameter schema, and end-to-end behavior through the MCP protocol.
...
- **Task 017** — GetPageImages service and DTOs (must be complete before this task)
...
Integration tests use test PDF files with **known, deterministic embedded images** generated by Task 017's `TestPdfGenerator` methods. The following test data is available:

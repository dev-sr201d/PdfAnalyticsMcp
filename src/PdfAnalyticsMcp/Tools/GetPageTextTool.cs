...
{
    [McpServerTool, Description("Returns text elements from a single PDF page with position, font, size, and color metadata. Each element includes a bounding box (x, y, w, h), font name, font size, and optional color/bold/italic flags. Use granularity 'words' (default) for layout analysis or 'letters' for character-level detail. When outputFile is provided, the full JSON result is written to that file and a compact summary is returned inline, enabling callers to handle large pages without payload limits.")]
    public string GetPageText(
        ...
        [Description("Optional absolute path to write the full JSON result to. When provided, the full result is written to this file instead of being returned inline, and a compact summary is returned.")] string? outputFile = null)
    {
        try
        {
